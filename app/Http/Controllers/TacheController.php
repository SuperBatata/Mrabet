...
class TacheController extends Controller
{
    public function index(){
        ...
        $phase1 = voyage::where('etat', 'phase1',)->get();
        $phase2 = voyage::where('etat', 'phase2',)->get();
    if (request()->ajax())
    {
    ...
    public function datatable_valider(){

        $valider = voyage::where('etat', 'terminé')->get();
        
    if (request()->ajax())
    {
        return DataTables::of($valider)
        ->addColumn('Action',function($row)
        {
            $btn =   '<a href="'.action('TacheController@fairePhase2',$row->idVoyage).'"
            class="btn btn-success btn btn-primary a-btn-slide-text"><span
        ...
        return view('users.tachev',compact('valider'));
